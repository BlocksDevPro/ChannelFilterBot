...
async def main():
    ...
    async with Client(config.SESSION_ID, config.API_ID, config.API_HASH, phone_number=config.PHONE_NUMBER) as client:
        resultText = ''
        for channel in channelsList:
            channelUsername = '@'+channel.replace('https://', '', 1).split('/')[1]
            ...
            try: 
                ...
                if 'FLOOD_WAIT_X' in str(telegram_error):
                    waitingSeconds = str(telegram_error).split('-')[1].split()[3]
                    print('waiting', waitingSeconds, 'before trying next channel!')
                    time.sleep(float(waitingSeconds))
                else: print(telegram_error)
            else:
                if channelInfo and channelInfo.linked_chat and str(channelInfo.linked_chat.type) == "ChatType.SUPERGROUP":
                    print("Fetched channel :", channelUsername, 'Discussion Group found.')
                    resultText += f"{channelUsername} : @{channelInfo.linked_chat.username}"
                else:
                    print("Fetched channel :", channelUsername, 'Discussion Group not found!')
